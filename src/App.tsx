...
export default function App() {
  ...
  return (
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 flex flex-col justify-between select-none bg-slate-950">
      {/* 顶部导航标题 */}
      <header className="max-w-4xl w-full mx-auto flex items-center justify-between mb-6">
        ...
        <div className="text-xs text-slate-400 bg-slate-900/80 border border-slate-800/80 rounded-lg px-3 py-1.5 backdrop-blur-md flex items-center gap-2">
          ...
          <span>Phase 4: UI 与状态捕获</span>
        </div>
      </header>

      {/* 主界面区域 (100% 居中宽屏影院级大卡片布局) */}
      <main className="max-w-4xl w-full mx-auto my-auto">
          ...
          <div className="glass-panel p-4 flex flex-col gap-4 relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between text-xs px-1 text-slate-400">
              <div className="flex items-center gap-2">
                ...
                {isFinished ? (
                  <>
                    ...
                    {finalNodeId && (
                      <span className="text-purple-400 font-medium font-mono">(终点: {finalNodeId})</span>
                    )}
                  </>
                ) : (
                  <>
              ...
              <div className="text-slate-500 font-mono text-[10px]">
                {isFinished
                  ? '剧情播放完毕'
                  : `URL: ${currentNode.videoUrl} | Duration: ${currentNode.duration}s`}
              </div>
            </div>
          </div>
        </section>
