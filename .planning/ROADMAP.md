...
- [x] **Phase 6: 体验升级与结构化章节目录** - 实现可交互的进度条（支持越过判定拦截）以及常驻在主界面右侧的毛玻璃结构化章节目录。
...
**Goal**: 实现视频时间轴跳转控制及拦截器，并构建可交互常驻的毛玻璃剧情分支目录，提升演示与调试便利度。
**Depends on**: Phase 5
**Requirements**: EXP-01, EXP-02
**Success Criteria**:

  1. 前台播放器的进度条能够被点击并跳转。
  2. 当跳转越过当前节点的任意交互判定点时，能够智能截断回退至该判定时刻，并在暂停时弹出分支选择卡片。
  3. 主界面右侧（宽屏下）能渲染一个章节目录组件，并能联动展示当前节点、已访问节点和未访问分支；点击任一节点可瞬移切换。

**Plans**: 2 plans
**Plans**:

- [ ] 06-01: 进度条可跳转及智能拦截机制实现
- [ ] 06-02: 剧本结构化目录组件开发与 App 左右分栏布局集成
...
| 6. 体验升级与结构化章节目录 | 2/2 | Complete | 2026-06-11 |
